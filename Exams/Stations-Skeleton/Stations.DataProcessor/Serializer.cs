...
namespace Stations.DataProcessor
{
	...
	{
        public static TripSatus TripStatus { get; private set; }
        ...
        public static string ExportDelayedTrains(StationsDbContext context, string dateAsString)
		{
            ...
            var delayedTrains = context.Trains
                .Include(e => e.Trips)
                .Where(e => e.Trips.Any(t => t.Status == Enum.Parse<TripSatus>("Delayed") && t.DepartureTime.Ticks <= date.Ticks))
                .Select(e => new
                {
                    TrainNumber = e.TrainNumber,
                    DelayedTrips = e.Trips
                    .Where(t => t.Status == Enum.Parse<TripSatus>("Delayed") && t.DepartureTime.Ticks <= date.Ticks)
                    .ToArray(),                                         
        }).Select(t=> new
        {
		...
		{
            var ticketsByCardType = context.Cards
        ...
        private static object riter(StringBuilder sb)
        {
